...
\indent UUIDv4 uses \texttt{crypto.\allowbreak randomUUID()} and \texttt{pg\_uuid\_v8} embeds an AES-encrypted timestamp. For the \texttt{dbkey} layout,
...
All 15 NIST SP~800-22 sub-tests~\cite{20} pass on three structured layouts (\texttt{struct-dbkey}, \texttt{multitenant}, and \texttt{eventsourcing}). An extended \texttt{dieharder}~\cite{21} battery, including \texttt{birthdays}, \texttt{rank\_32x32}, \texttt{dna}, \texttt{count\_1s\_str}, \texttt{parking\_lot}, \texttt{runs}, \texttt{sts\_monobit}, and \texttt{sts\_serial}, comprising 38 sub-tests, 5 independent trials each, and 100M-bit samples, reports 152/152 PASS (modal assessment per sub-test) across four generator variants: native UUIDv4, \texttt{rawv8} (RFC~9562 v8 without GA), \texttt{genoid-v8} (GA + pooling), and \texttt{struct-dbkey} (structured). The GA and non-GA variants pass identically, which is the empirical counterpart to the Section~\ref{sec:entropy} proof: GA neither degrades nor improves statistical quality on random-type fields.

\subsection{Throughput and baseline comparison}

Table~\ref{tab:throughput} reports throughput across all tested generators.
...
Table~\ref{tab:pguuid-vs-genoid} summarizes the capability differences.

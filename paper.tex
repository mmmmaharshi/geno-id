...
Every branch assigns $v'$ unconditionally, so the loop body cannot fail and the procedure contains no backward jump: it executes exactly $k$ iterations for $k$ constrained fields, independent of the drawn value. Correctness is by construction: each branch's codomain is the constraint's satisfying set.
...
secondary index. The 12-bit enum bounds the scheme at 4096 tenants;
larger populations require widening the field and re-declaring.
...
const multitenant: V8Layout = {
  ...
  fields: [
    { name: "tenant", offset:  0, length: 12, type: "shard",
      constraint: { kind: "range", min: 0, max: 4095 } },
    { name: "region", offset: 12, length:  8, type: "node",

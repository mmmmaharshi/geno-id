...
With $\ell_c=16$ and $g=1$, reseeding leaves at least $2^{15}=32768$ increments per millisecond. At the measured 1.81M~ops/sec (Table~\ref{tab:throughput}), a generator emits $\approx$1810 UUIDs per millisecond, an $18\times$ margin before the rollover path is reached. State $M$ is per-generator-instance; concurrent generators each hold an independent $M$, so \texttt{repairConstraints} requires no locks.

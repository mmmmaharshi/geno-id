...
\begin{itemize}
    ...
    \item Field-boundary crossover over two independently pooled CSPRNG parents, proved to preserve min-entropy (a worst-case measure of unpredictability: how hard the most likely guess is for an attacker) on random-type fields (Sect.~\ref{sec:entropy}).
...
cost grows in a straight line. The proof below makes that precise.}
...
\caption{Throughput by generator (Ubuntu/Bun, mean of 10 trials). GenoID's pooled
v8 nears native v4, and \texttt{genoid-structured} edges past \texttt{pg\_uuid\_v8}
and \texttt{ulid-v8} while adding declarative multi-field composition.}

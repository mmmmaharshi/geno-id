...
// Fast local smoke variant of scripts/export-dieharder.ts. Exports small
// raw-binary bitstreams (packed bytes, not ASCII "0"/"1") so the dieharder
// pipeline can be exercised end-to-end without waiting on the full 12.5MB
// export. NIST SP 800-22 (scripts/nist-bridge.py) validates on ~1.22M bits per
// sample; this smoke script only targets 200K bits per generator.
//
// Budget: 200,000 bits (~25KB) per flat generator — enough to confirm the
// export/bit-packing path works, not enough for meaningful dieharder p-values.
...
// fast local smoke: 200K bits (~25KB) instead of the full 12.5MB export
const TARGET_BITS = 200_000

console.log(
  `Exporting dieharder smoke bitstreams (${(TARGET_BITS / 1e6).toFixed(2)}M bits target per generator)...`,
)

...
// Exports large raw-binary bitstreams (packed bytes, not ASCII "0"/"1") for
// the dieharder battery. NIST SP 800-22 (scripts/nist-bridge.py) validates on
// ~1.22M bits per sample, which is enough for the SP 800-22 recommended
// minimums but far short of what dieharder's harder tests (e.g.
// rgb_lagged_sum, diehard_bitstream) want before the file has to be rewound.
// Rewinding a short file re-uses bits and invalidates the p-values, so this
// script targets a much larger sample per generator, writing directly into a
// preallocated byte buffer (no giant intermediate strings).
//
// Budget: 100,000,000 bits (12.5 MB) for the flat generators (v4, raw-v8,
// GenoID-pooled) and a proportionally larger UUID count for the structured
// dbkey layout, which yields only 50 random bits/UUID.
...
// Avoids building a multi-hundred-megabyte intermediate ASCII string.
class BitWriter {
  buf: Uint8Array
  bitPos = 0
...
console.log(
  `Exporting dieharder-scale bitstreams (${(TARGET_BITS / 1e6).toFixed(0)}M bits target per generator)...`,
)
await exportFlat("v4", genV4Native, TARGET_BITS)
await exportFlat("rawv8", genRawV8, TARGET_BITS)
await exportFlat("genoid", genGenoID, TARGET_BITS)
...
await exportStructured("struct-dbkey", dbkey, TARGET_BITS)

...
const { genV4Native, genGenoID, toUuidString } = algo
...
async function generateSamples(
  ...
  const file = await writeBitsFile(`${label}.bits.txt`, allBits)
  console.log(
    `Wrote ${(allBits.length / 1000000).toFixed(1)}M bits to ${file}`,
  )
